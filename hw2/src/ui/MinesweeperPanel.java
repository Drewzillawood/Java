...
public class MinesweeperPanel extends JPanel implements CellObserver
{
	 ...
	 */
	private volatile boolean	animating;					// flag shared with
															// helper threads
	...
	/**
	 * When animation is in effect, receives notification when a cell status is
	 * modified.
	 */
	@Override
	...
	public void paintComponent(Graphics g)
	{
		...
		for(int row = 0; row < game.getRows(); ++row)
		{
			for(int col = 0; col < game.getColumns(); ++col)
			{
				...
				else if(c.getStatus() == Status.HIDDEN && c.getMark() == Mark.QUESTION_MARK)
				{
					g.setColor(Color.BLACK);
					drawText(g, row, col, "?");
				}
	...
	private boolean isExploring(Cell c)
	{
		...
		return(s == Status.EXPLORE_UP || s == Status.EXPLORE_LEFT || s == Status.EXPLORE_DOWN
				|| s == Status.EXPLORE_RIGHT);
	}

	/**
	 * Draws a line from the center of each cell in EXPLORING state to indicate
	 * the direction in which it is exploring.
	 * 
	 * @param g
	 */
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			if(!animating)
			{
				...
				if(e.getButton() == MouseEvent.BUTTON1)
				{
					...
					{
						// if it's a cell with count 0, run in worker thread to
						// get animation
						animating = true;
	...
	/**
	 * Timer callback updates count of the elapsed seconds, except when
	 * animating.
	 */
